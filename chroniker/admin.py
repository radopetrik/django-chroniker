...
#            fields.append('is_running')
    ...
    def get_frequency(self, obj=None):
        ...
        if obj.params:
            return mark_safe("%s (%s)" % (freq, obj.params))
        return freq
    ...
    def run_button(self, obj=None):
        ...
        return mark_safe('<a href="{url}" class="button">Run</a>'.format(**kwargs))
    ...
    def view_logs_button(self, obj=None):
        ...
        return mark_safe('<a href="{url}?job__id__exact={id}" target="_blank" class="button">View&nbsp;{count}</a>'\
            .format(**kwargs))
